...
@Slf4j
/**
 * @description HospitalServiceImpl-hospital Service Implementation is
 *              responsible for the methods under the hospital entity
 * @author Janani.V
 * @since 11-12-2019
 *
 */
public class HospitalServiceImpl implements HospitalService {
	...
	/**
	 * getAllHospital- In this method ,we will get all the hospitalList from
	 * database.
	 * 
	 * @return List of the hospitalList using HospitalDto hospitalRepository is used
	 *         to get the findall method from the jpa repository
	 */
	@Override
	public List<HospitalDto> getAllHospital() {
		List<Hospital> hospital = hospitalRepository.findAll();
		log.info(" getting city details");
		return hospital.stream().map(this::convertEntityToDto).collect(Collectors.toList());
